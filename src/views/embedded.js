...
export class EmbeddedDistrictr {
    constructor(target, module, options) {
        ...
        fetch(module.url)
            ...
            .catch(e => {
                console.error(e);
                render(html`<h4>An error occurred.</h4>`, targetElement);
            });
    }
    ...
    render() {
        if (!this.toolbarTarget || !this.toolbar) return;
    ...
    checkConnected(msg_box) {
        ...
        const assignment = this.state.plan.assignment;
        let visited = {};
        let total = 0;
        ...
        const walkNeighborhood = (visited, node) => {
            let count = 1;
            visited[node] = true;
            for (let nbor of this.graph[node]) {
                if (visited[nbor] === false) {
                    count += walkNeighborhood(visited, nbor);
                }
            }
            return count;
        };

        const root = this.homeBlock.id;
        const found = walkNeighborhood(visited, root);
    ...
    loadAddress(str, searchBox) {
        const url = `https://api.mapbox.com/geocoding/v5/mapbox.places/${encodeURIComponent(str)}.json?autocomplete=false&limit=1&bbox=${this.bounds[0][0]},${this.bounds[0][1]},${this.bounds[1][0]},${this.bounds[1][1]}&access_token=${mapboxgl.accessToken}`;
        ...
        fetch(url)
            ...
            .then(d => {
                ...
                if (searchBox) searchBox.value = d.features[0].place_name;
                ...
                let colored = false;
                const colorBlock = () => {
                    let block = this.map.queryRenderedFeatures(this.map.project(center), { layers: [this.state.units.id], validate: false })[0];
                    if (!block || colored) return;
                    colored = true;

                    if (this.homeBlock) {
                        this.map.setFeatureState(this.homeBlock, { ...this.homeBlock.state, home: false });
                        this.clearNeighborhood(false);
                    }
                    ...
                    this.map.setFeatureState(block, { ...block.state, home: true });
                };
                ...
                // Zoom, respecting minimum state zoom
                const MIN_STATE_ZOOM = 7.5;
                this.map.easeTo({
                    center,
                    zoom: Math.max(this.zoomTo, MIN_STATE_ZOOM),
                    duration: 1000
                });
            });
    }
    ...
    clearNeighborhood(keepHome = true) {
        const ids = Object.keys(this.state.plan.assignment);
        const features = this.state.units.querySourceFeatures().filter(x => ids.includes(x.id));
        features.forEach(x => this.state.units.setAssignment(x, null));
        ids.forEach(x => { this.state.plan.assignment[x] = null; });
...
// --- window globals ---
window.MapDraw = (target, module, options) => new EmbeddedDistrictr(target, module, options);

window.showError = function(msg, sel = "#ns__msg-search") {
    const msg_box = document.querySelector(sel);
    if (msg !== null) {
        msg_box.hidden = false;
    ...
    return {
        "fill-color": ["interpolate-hcl",
            ...
            ["case", ["==", opts.denominator || ["get", "pop"], 0],
                opts.midpt || 0.5,
                ["/", opts.numerator || ["get", "gop"], opts.denominator || ["get", "pop"]],
            ],
...
window.UnivariateOverlay = function(opts) {
    return {
        "fill-color": ["interpolate-hcl",
            ...
            ["case", ["==", opts.denominator || ["get", "pop"], 0],
                0,
                ["/", opts.numerator || ["get", "dem"], opts.denominator || ["get", "pop"]],
            ],
